...
public class FansNetworkScraper extends AbstractScraper {
    public FansNetworkScraper() {
        setPostPattern("</b> on ([0-9]+:[0-9]+ - [A-Z][a-z]{2} [0-9]+) with [0-9]+ views</td>" +
                "[\\S\\s]*?<td valign=\"top\">([\\S\\s]*?)</div></td>" +
                "[\\S\\s]*?<a href=\"/football/swanseacity/settings/ignore-member/([\\S\\s]*?) title");
